...
Never commit large datasets or proprietary paths; only Torch tensors derived from synthetic data belong in tests. Respect the restricted network policy by keeping downloads out of runtime code. Ensure CUDA-specific functionality still runs on CPU-only machines (avoid hard `cuda` dependencies in core APIs).
