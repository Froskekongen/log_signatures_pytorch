...
def _explicit_windowed_signature(path: torch.Tensor, depth: int, window_size: int, hop_size: int) -> torch.Tensor:
    """Reference implementation using explicit window batching."""
    windows = path.unfold(dimension=1, size=window_size, step=hop_size)  # (batch, num_windows, width, window_size)
...
def test_windowed_signature_matches_explicit(window_size: int, hop_size: int):
    ...
    actual = windowed_signature(path, depth=depth, window_size=window_size, hop_size=hop_size)
    expected = _explicit_windowed_signature(path, depth=depth, window_size=window_size, hop_size=hop_size)
